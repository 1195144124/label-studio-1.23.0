...
export const ViewAllToggle = observer(
  ({ isActive, onClick }: ViewAllToggleProps) => {
    return (
      <button
        type="button"
      ...
      >
        <div
          className={cn("view-all-toggle").elem("mainSection").toClassName()}
        >
          <div
            className={cn("view-all-toggle")
              .elem("iconContainer")
          ...
          <div className={cn("view-all-toggle").elem("content").toClassName()}>
            <Typography
              variant="label"
              size="small"
              className={cn("view-all-toggle").elem("label").toClassName()}
            >
              比较所有标注
            </Typography>
          </div>
